...
int main(int argc, char* argv[]) {
	...
	// Create canvas_color vector to store color of each pixel
	...
	// dynamically allocate memory of size `height_px` for each row
	...
	for (int i = 0; i <= input_manage.GetImageSize()[1] - 1; i++) {
		for (int j = 0; j <= input_manage.GetImageSize()[0] - 1; j++) {
			...
			// std::cout << ray.direction() << ray.Length() << std::endl;

			// Color grid_color = output_grid(input_manage, pixel_upper_left);

			Color obj_color = ray.RayTrace(input_manage, ray_dir.Length());
